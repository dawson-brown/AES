...
MU_TEST(test_aes_enc) {
    ...
    uint8_t text[AES_BLOCK_LENGTH] = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
    uint8_t key[AES_BLOCK_LENGTH] = "\x10\xa5\x88\x69\xd7\x4b\xe5\xa3\x74\xcf\x86\x7c\xfb\x47\x38\x59";
    uint8_t expected[AES_BLOCK_LENGTH] = "\x6d\x25\x1e\x69\x44\xb0\x51\xe0\x4e\xaa\x6f\xb4\xdb\xf7\x84\x65";

    aes_128_enc(text, key);
    mu_assert(memcmp(text, expected, AES_BLOCK_LENGTH) == 0, "aes encryption: failed");    
}

MU_TEST(test_aes_dec) {

    uint8_t text[AES_BLOCK_LENGTH] = "\x5d\x9b\x05\x57\x8f\xc9\x44\xb3\xcf\x1c\xcf\x0e\x74\x6c\xd5\x81";
    uint8_t key[AES_BLOCK_LENGTH] = "\xb6\x36\x4a\xc4\xe1\xde\x1e\x28\x5e\xaf\x14\x4a\x24\x15\xf7\xa0";
    uint8_t expected[AES_BLOCK_LENGTH] = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";

    aes_128_dec(text, key);
...
MU_TEST(test_ecb_enc) {
    ...
    ecb_enc(aes_128_enc, text, 2*AES_BLOCK_LENGTH, key);
    mu_assert(memcmp(text, expected, 32) == 0, "ecb encryption: failed");
}
...
MU_TEST(test_ecb_dec) {
    ...
    ecb_dec(aes_128_dec, text, 2*AES_BLOCK_LENGTH, key);
    mu_assert(memcmp(text, expected, 32) == 0, "ecb decryption: failed");
}
...
MU_TEST(test_cbc_enc){
    ...
    cbc_enc(aes_128_enc, text, 2*AES_BLOCK_LENGTH, key, iv, AES_BLOCK_LENGTH);
    mu_assert(memcmp(text, expected, 32) == 0, "cbc encryption: failed");
}
...
MU_TEST(test_cbc_dec){
    ...
    cbc_dec(aes_128_dec, text, 2*AES_BLOCK_LENGTH, key, iv, AES_BLOCK_LENGTH);
    mu_assert(memcmp(text, expected, 32) == 0, "cbc decryption: failed");
}
