...
aes_t set_state(unsigned char *plaintext){
  aes_t state;
  state.state = (unsigned char *)malloc(STATE_SIZE);
  memcpy(state.state, plaintext, STATE_SIZE);
  return state;
}
...
void sub_bytes(unsigned char *state){
  ...
  unsigned char i;
  for (i=0; i<STATE_SIZE; i+=NB){

    SBOX(state[i], state[i+1], state[i+2], state[i+3]);
...
void shift_rows(unsigned char *state){
  ...
  unsigned char temp;
  SHIFT_1(temp, state[1], state[5], state[9], state[13]);
  SHIFT_2(temp, state[2], state[6], state[10], state[14]);
  SHIFT_3(temp, state[3], state[7], state[11], state[15]);

}

//largely the work of wikipedia (yet to learn all the finite field business)
...
void aes(aes_t *state, unsigned char *key){
  ...
  unsigned char i;
  add_round_key(state->state, key);      
  key+=STATE_SIZE;

  for(i=0; i<(NR-1); i++){
    sub_bytes(state->state);
    shift_rows(state->state);
    mix_columns(state->state);
    add_round_key(state->state, key);
    key+=STATE_SIZE;
  }
